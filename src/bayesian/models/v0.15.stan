// v0.15: Keeps the v0.13 population hierarchy and adds a sparse downward
// shift on log-rho for both providers to target low-penetration outlier towers.
data {
...
parameters {
  ...
  real<lower=0> sigma_rho1; // regular tower-level sd for provider 1
  real<lower=0> sigma_rho2; // regular tower-level sd for provider 2
  real<lower=0> sigma_drop1; // downward-shift sd for provider 1
  real<lower=0> sigma_drop2; // downward-shift sd for provider 2
  real<lower=0> radius_rho1; // provider 1 distance radius
  real<lower=0> radius_rho2; // provider 2 distance radius
  vector[J1] z_rho1; // regular tower effects for provider 1
  vector[J2] z_rho2; // regular tower effects for provider 2
  vector<lower=0>[J1] delta_rho1; // downward shift magnitude for provider 1
  vector<lower=0>[J2] delta_rho2; // downward shift magnitude for provider 2
  vector<lower=0, upper=1>[J1] omega_rho1; // sparsity weight for provider 1 towers
  vector<lower=0, upper=1>[J2] omega_rho2; // sparsity weight for provider 2 towers
...
transformed parameters {
  ...
  phi_housing = exp(
                    alpha_phi_housing + nbr_phi_housing[hh]
                    + X * beta_housing);
  ...
  for (i in 1 : I) {
    real n_i = 0;
    if (tents[i] > 0) {
      n_i += tents[i] * phi_tents[i];
    }
    if (housing[i] > 0) {
      n_i += housing[i] * phi_housing[i];
    }
    N[i] = n_i;
  }
...
model {
  ...
  z_rho1 ~ std_normal();
  z_rho2 ~ std_normal();
  sigma_rho1 ~ normal(0, 0.25);
  sigma_rho2 ~ normal(0, 0.25);
  sigma_drop1 ~ normal(0, 0.6);
  sigma_drop2 ~ normal(0, 0.6);
  delta_rho1 ~ normal(0, sigma_drop1);
  delta_rho2 ~ normal(0, sigma_drop2);
  omega_rho1 ~ beta(1, 9);
  omega_rho2 ~ beta(1, 9);
  ...
  sigma_mun_phi_tents ~ normal(0, 0.5);
  sigma_nbr_phi_tents ~ normal(0, 0.5);
  ...
  sigma_mun_phi_housing ~ normal(0, 0.5);
  sigma_nbr_phi_housing ~ normal(0, 0.5);
