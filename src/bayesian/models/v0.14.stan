// v0.14: Keeps the v0.13 population hierarchy and adds a sparse continuous
// outlier channel for tower-level rho for both providers.
data {
...
parameters {
  ...
  real<lower=0> sigma_rho1_out; // outlier tower-level sd for provider 1
  real<lower=0> sigma_rho2_out; // outlier tower-level sd for provider 2
  ...
  vector[J1] z_rho1_out; // outlier tower effects for provider 1
  vector[J2] z_rho2_out; // outlier tower effects for provider 2
  vector<lower=0, upper=1>[J1] omega_rho1; // outlier weight for provider 1 towers
  vector<lower=0, upper=1>[J2] omega_rho2; // outlier weight for provider 2 towers
...
transformed parameters {
  ...
  rho1 = exp(
    alpha_rho1
    + (1 - omega_rho1) .* (sigma_rho1 * z_rho1)
    + omega_rho1 .* (sigma_rho1_out * z_rho1_out)
  );
  rho2 = exp(
    alpha_rho2
    + (1 - omega_rho2) .* (sigma_rho2 * z_rho2)
    + omega_rho2 .* (sigma_rho2_out * z_rho2_out)
  );
...
model {
  ...
  z_rho1 ~ std_normal();
  z_rho2 ~ std_normal();
  z_rho1_out ~ std_normal();
  z_rho2_out ~ std_normal();
  sigma_rho1 ~ normal(0, 0.25);
  sigma_rho2 ~ normal(0, 0.25);
  sigma_rho1_out ~ normal(0, 0.6);
  sigma_rho2_out ~ normal(0, 0.6);
  omega_rho1 ~ beta(1, 9);
  omega_rho2 ~ beta(1, 9);
