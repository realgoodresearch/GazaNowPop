...
model_data <- function(
  ...
  I_g <- c()
  for (g in 1:G) {
    I_g[g] <- sum(gov_vect == g)
  }

  grids_by_gov <- matrix(0, nrow = G, ncol = max(I_g))
  for (g in 1:G) {
    grids_by_gov[g, 1:I_g[g]] <- which(gov_vect == g)
  }
  ...
  I_m <- c()
  for (m in 1:M) {
    I_m[m] <- sum(mun_vect == m)
  }

  grids_by_mun <- matrix(0, nrow = M, ncol = max(I_m))
  for (m in 1:M) {
    grids <- which(mun_vect == m)
    if (length(grids) > 0) {
  ...
  I_h <- c()
  for (h in 1:H) {
    I_h[h] <- sum(nbr_vect == h)
  }

  grids_by_nbr <- matrix(0, nrow = H, ncol = max(I_h))
  for (h in 1:H) {
    grids <- which(nbr_vect == h)
    if (length(grids) > 0) {
      grids_by_nbr[h, 1:I_h[h]] <- which(nbr_vect == h)
    }
  }
...
init_generator <- function(md) {
  list(
    ...
    phi_tents = runif(1, 0, 20),
    phi_housing = runif(1, 0, 20)
  )
}
