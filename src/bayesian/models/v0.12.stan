// v0.12: Restores neighbourhood-level population effects while simplifying
// tower-level rho to a symmetric Gaussian hierarchy for both providers.
data {
...
model {
  ...
  z_rho1 ~ std_normal();
  z_rho2 ~ std_normal();
  sigma_rho1 ~ normal(0, 0.15);
  sigma_rho2 ~ normal(0, 0.15);
  ...
  sigma_nbr_phi_tents ~ cauchy(0, 1);
  ...
  sigma_nbr_phi_housing ~ cauchy(0, 1);
