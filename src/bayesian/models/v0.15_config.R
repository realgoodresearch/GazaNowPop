...
init_generator <- function(md) {
  list(
    ...
    sigma_drop1 = abs(rnorm(1, 0, 0.15)),
    sigma_drop2 = abs(rnorm(1, 0, 0.15)),
    radius_rho1 = exp(rnorm(1, log(3000), 0.1)),
    radius_rho2 = exp(rnorm(1, log(3000), 0.1)),
    z_rho1 = rnorm(md$J1, 0, 0.1),
    z_rho2 = rnorm(md$J2, 0, 0.1),
    delta_rho1 = abs(rnorm(md$J1, 0, 0.05)),
    delta_rho2 = abs(rnorm(md$J2, 0, 0.05)),
    omega_rho1 = rbeta(md$J1, 1, 20),
    omega_rho2 = rbeta(md$J2, 1, 20),
    ...
    sigma_nbr_phi_tents = exp(rnorm(1, log(0.05), 0.1)),
    z_gov_phi_tents = rnorm(md$G, 0, 0.1),
    z_mun_phi_tents = rnorm(md$M, 0, 0.1),
    z_nbr_phi_tents = rnorm(md$H, 0, 0.1),
    ...
    sigma_nbr_phi_housing = exp(rnorm(1, log(0.05), 0.1)),
    z_gov_phi_housing = rnorm(md$G, 0, 0.1),
    z_mun_phi_housing = rnorm(md$M, 0, 0.1),
    z_nbr_phi_housing = rnorm(md$H, 0, 0.1),
